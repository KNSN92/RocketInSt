...
export type TimeAsMinutesWithWeekday = { minutes: TimeAsMinutes, weekday: number }
...
export function getNowJSTTimeAsMinutesWithWeekday(): TimeAsMinutesWithWeekday {
    const nowJSTTime = new Date(new Date().getTime() + (JSTOffset * 3600 * 1000))
    return { weekday: nowJSTTime.getUTCDay(), minutes: nowJSTTime.getUTCHours() * 60 + nowJSTTime.getUTCMinutes() }
}
